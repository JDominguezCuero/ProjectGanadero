            ...
            <div class="container">
                ...
                <div class="product-list-container">
                    ...
                    <aside class="filters-sidebar" data-aos="fade-right">
                        ...
                        <form action="{{ route('productos.index') }}" method="GET"> 
                            ...
                            <div class="filter-group">
                                ...
                                <input type="text" name="buscar" id="buscar" placeholder="Nombre o descripción" value="{{ $filtro_busqueda ?? '' }}">
                            </div>

                            <div class="filter-group">
                                <label>Precio:</label>
                                <input type="number" name="precio_min" placeholder="Mín." value="{{ $filtro_precio_min ?? '' }}">
                                <input type="number" name="precio_max" placeholder="Máx." value="{{ $filtro_precio_max ?? '' }}">
                            </div>

                            <div class="filter-group">
                                <label for="ordenar_por">Ordenar por:</label>
                                <select name="ordenar_por" id="ordenar_por" onchange="this.form.submit()">
                                    <option value="fecha_reciente" {{ $ordenar_por == 'fecha_reciente' ? 'selected' : '' }}>Fecha (más reciente)</option>
                    ...
                    <main style="margin-top: auto; padding: 5px;" class="main-content" data-aos="fade-left">
                        @if (!empty($productos_por_categoria_en_listado))
                            @foreach ($productos_por_categoria_en_listado as $nombre_categoria => $productos)
                                @if (!empty($productos))
                                    <div class="category-section">
                                        <h2>{{ $nombre_categoria }}</h2>
                                        <div class="products-grid">
                                            {{-- Renderizado de los productos (usa un componente o partial) --}}
                                            @include('partials.product_items', ['productos' => $productos])
                                        </div>
                                    </div>
                                @endif
                            @endforeach
